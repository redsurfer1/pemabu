...
BEGIN
  IF EXISTS (
    ...
    WHERE table_schema = 'public' AND table_name = 'sleeve_holdings'
  ) THEN
...
-- ── 4–5. model_assumptions: DELETE policy + sleeve_type (when table exists) ──
DO $model$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.tables
    WHERE table_schema = 'public' AND table_name = 'model_assumptions'
  ) THEN
    RAISE NOTICE 'model_assumptions not present — skipping RLS/sleeve_type pass.';
    RETURN;
  END IF;
  ...
  IF NOT EXISTS (
    ...
    WHERE conname = 'model_assumptions_sleeve_type_check'
  ) THEN
    ALTER TABLE public.model_assumptions
      ADD CONSTRAINT model_assumptions_sleeve_type_check
