...
-- Skips gracefully when model_assumptions was never created on this database.
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE table_schema = 'public'
      AND table_name = 'model_assumptions'
  ) THEN
    RAISE NOTICE 'model_assumptions not present — skipping factor column expansion.';
    RETURN;
  END IF;
