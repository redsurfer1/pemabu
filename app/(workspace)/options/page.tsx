...
export default async function OptionsPage({
  ...
  searchParams: Promise<{ portfolio_id?: string; demo?: string }>;
}) {
  const sp = await searchParams;
  const demo = isDemoRequest(sp);
  if (!demo) await requireServiceAccess("addon_options_overlay");
  ...
  return (
    <div className="min-h-screen bg-[#0A1628] px-4 py-8 sm:px-8">
      <OptionsOverlayClient portfolioId={portfolioId} demo={demo} />
    </div>
  );
}
