/**
 * Execution vault — routes to local Postgres or Supabase cloud
 * based on USE_LOCAL_VAULT env var.
 *
 * USE_LOCAL_VAULT=true  → direct pg pool (self-hosted Docker vault)
 * USE_LOCAL_VAULT=false → supabaseAdmin RPCs (cloud-hosted vault)
 */
...
async function withCloud<T>(label: string, cloudFn: () => Promise<T>, localFn: () => Promise<T>): Promise<T> {
  if (isLocalVaultExecutionPlane()) return localFn();
  try {
    return await cloudFn();
  } catch (err) {
    console.warn(`[vault] cloud fallback "${label}" failed, trying local vault:`, err);
    return localFn();
  }
}

// ── Import cloud plane lazily (avoids circular imports in tests) ──────────

async function cloudPlane() {
  return import("@/lib/execution/cloud-vault-plane");
}

// ── Public API — same signatures as before ────────────────────────────────

export async function insertHoldingAuditVault(row: HoldingAuditInsert): Promise<void> {
  return withCloud("insertHoldingAudit", async () => {
    const cloud = await cloudPlane();
    await cloud.insertHoldingAuditCloud(row);
  }, async () => {
    await pool().query(
      ...
      [
        row.userId, row.portfolioId, row.sleeveId, row.holdingId,
        row.eventType, row.ticker, row.quantityBefore, row.quantityAfter,
        row.costBasisBefore, row.costBasisAfter, JSON.stringify(row.notes ?? {}),
      ],
    );
  });
}
...
export async function ensureExecutionControlRowVault(userId: string): Promise<ExecutionControlRow> {
  return withCloud("ensureExecutionControl", async () => {
    const cloud = await cloudPlane();
    return cloud.ensureExecutionControlRowCloud(userId);
  }, async () => {
    const { rows } = await pool().query<Record<string, unknown>>(
      `SELECT * FROM execution_control WHERE user_id = $1::uuid`, [userId],
    );
    if (rows[0]) {
      const r = rows[0];
      return {
...
export async function portfolioNavUsdVault(portfolioId: string): Promise<ReturnType<typeof d>> {
  return withCloud("portfolioNavUsd", async () => {
    const cloud = await cloudPlane();
    return cloud.portfolioNavUsdCloud(portfolioId);
  }, async () => {
...
export async function rolling24hNotionalUsdVault(userId: string): Promise<ReturnType<typeof d>> {
  return withCloud("rolling24hNotional", async () => {
    const cloud = await cloudPlane();
    return cloud.rolling24hNotionalUsdCloud(userId);
  }, async () => {
...
export async function buildGuardrailContextVault(
  ...
  const [control, portfolioNav, rolling24hNotional] = await Promise.all([
    ensureExecutionControlRowVault(userId),
    portfolioNavUsdVault(portfolioId),
    rolling24hNotionalUsdVault(userId),
  ]);
...
export async function fetchTradeProposalVault(
  ...
  return withCloud("fetchTradeProposal", async () => {
    const cloud = await cloudPlane();
    return cloud.fetchTradeProposalCloud(userId, proposalId);
  }, async () => {
    const { rows } = await pool().query<Record<string, unknown>>(
...
export async function updateTradeProposalStatusVault(proposalId: string, status: string): Promise<void> {
  return withCloud("updateTradeProposalStatus", async () => {
    const cloud = await cloudPlane();
    await cloud.updateTradeProposalStatusCloud(proposalId, status);
  }, async () => {
    await pool().query(
...
export async function fetchSleeveHoldingQtyPriceVault(
  holdingId: string,
): Promise<{ qty: string; price_seed: string } | null> {
  return withCloud("fetchSleeveHoldingQtyPrice", async () => {
    const cloud = await cloudPlane();
    return cloud.fetchSleeveHoldingQtyPriceCloud(holdingId);
  }, async () => {
...
export async function fetchExchangeCredentialsVault(
  ...
  return withCloud("fetchExchangeCredentials", async () => {
    const cloud = await cloudPlane();
    const data = await cloud.fetchExchangeCredentialsCloud(userId, exchangeName);
    return data as ExchangeCredentialVaultRow | null;
  }, async () => {
...
export async function updateExecutionControlVault(
  ...
  return withCloud("updateExecutionControl", async () => {
    const cloud = await cloudPlane();
    await cloud.updateExecutionControlCloud(userId, fields);
  }, async () => {
    const entries = Object.entries(fields).filter(([, v]) => v !== undefined) as [
      keyof typeof fields, string | number | boolean | string[],
    ][];
    if (entries.length === 0) {
    ...
    vals.push(userId);
    await pool().query(`UPDATE execution_control SET ${sets.join(", ")} WHERE user_id = $${i}::uuid`, vals);
  });
}
...
export async function insertDailyExecutionLogVault(input: {
  ...
  return withCloud("insertDailyExecutionLog", async () => {
    const cloud = await cloudPlane();
    await cloud.insertDailyExecutionLogCloud(input);
  }, async () => {
    await pool().query(
...
export async function deleteExchangeCredentialsVault(
  ...
  return withCloud("deleteExchangeCredentials", async () => {
    const cloud = await cloudPlane();
    await cloud.deleteExchangeCredentialsCloud(userId, exchangeName);
  }, async () => {
    await pool().query(
...
export async function upsertExchangeCredentialsVault(row: {
  ...
  return withCloud("upsertExchangeCredentials", async () => {
    const cloud = await cloudPlane();
    await cloud.upsertExchangeCredentialsCloud(row);
  }, async () => {
    await pool().query(
      `INSERT INTO exchange_credentials (
       ...
       ON CONFLICT (user_id, exchange_name) DO UPDATE SET
         ...
         updated_at = now()`,
      [row.userId, row.exchange, row.encrypted_api_key, row.encrypted_secret,
       row.iv, row.auth_tag, row.secret_iv, row.secret_auth_tag],
    );
  });
}
...
> {
  return withCloud("listPendingTradeProposals", async () => {
    const cloud = await cloudPlane();
    return cloud.listPendingTradeProposalsCloud(userId);
  }, async () => {
    ...
    return rows as Array<{
      id: string; portfolio_id: string; sleeve_id: string;
      holding_id: string | null; ticker: string; quantity: string;
    }>;
  });
}

export async function setPortfolioAutonomousExecutionVault(
  portfolioId: string, userId: string, enabled: boolean,
): Promise<boolean> {
  return withCloud("setAutonomousExecution", async () => {
    const cloud = await cloudPlane();
    return cloud.setPortfolioAutonomousExecutionCloud(portfolioId, userId, enabled);
  }, async () => {
    const { rowCount } = await pool().query(
      `UPDATE portfolios SET autonomous_execution_enabled = $3, updated_at = now()
...
export async function listTradeProposalsForUserVault(
  ...
  return withCloud("listTradeProposalsForUser", async () => {
    const cloud = await cloudPlane();
    return cloud.listTradeProposalsForUserCloud(userId, limit);
  }, async () => {
    const { rows } = await pool().query(
